...
class webSpider(threading.Thread):
	...
	def __init__(self, linkQueue, linkDepth, masterList):
...
#		self.thread = i
	...
	def run(self):
		current_depth = 0
		print "CURRENT DEPTH START: " + str(current_depth)
		print "LINK DEPTH START: " + str(self.linkDepth)
		print '\n'

		"""function that attempts to login to an ftp site"""
		# keeps the thread running until the Queue is empty
		while not self.linkQueue.empty() and current_depth <= self.linkDepth:
#			print "THREAD NUMBER: " + str(self.thread)
...
#			try:
			if depth == current_depth:
				...
				self.checkList(current_link_list, current_depth + 1)
		#		current_depth += 1
			else:
				self.linkQueue.put((url, depth))
				current_depth += 1
#			except:
#				print "[DEBUG] BROKEN"
...
#		# calls the class ftpLogin sending the Queue and number range
		workerThread = webSpider(linkQueue, depth, masterList)
