...
// Custome user struct to store information about users
struct user {
	int uid;
	int credits;
	int highscore;
...
int main() {
	...
	while (choice != 7) {
		...
		else if (choice < 4) {		   // Otherwise, the choice was a game of some sort
...
// The is a new user registration function.
...
// It is used primarily for updating credits after games.
void update_player_data() {
	int fd, i, read_uid;
	char burned_byte;
...
// This functionsimply awards the jackpot for the Pick a Number game.
void jackpot() {
	printf("*+*+*+*+*+* JACKPOT *+*+*+*+*+*\n");
...
void input_name() {
	...
	while (input_char != '\n') {	   // Loop until newline
		*name_ptr = input_char;    // Dereference name_ptr and put input char
...
// for the second wager in the Find the Ace game.  This function
...
int find_the_ace() {
	...
	printf("As this point, you may either select a different card or\n");
	...
	while (invalid_choice) {  // Loop until valid choide is made
